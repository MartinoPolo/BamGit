...
pub fn get_overview_data(
    ...
    let query = format!(
        "SELECT
            ...
            (SELECT SUM(s_td.cost_usd) FROM sessions s_td JOIN issues i_td ON s_td.issue_id = i_td.id WHERE i_td.dashboard_id = d.id AND date(s_td.started_at) = date('now')) AS cost_today_usd,
            (SELECT SUM(s_wk.cost_usd) FROM sessions s_wk JOIN issues i_wk ON s_wk.issue_id = i_wk.id WHERE i_wk.dashboard_id = d.id AND s_wk.started_at >= datetime('now', '-7 days')) AS cost_week_usd,
    ...
    let data = statement
        .query_map([], |row| {
            let status_string: String = row.get(22)?;
            let status =
            ...
            Ok(OverviewWorkspaceData {
                ...
                cost_today_usd: row.get(10)?,
                cost_week_usd: row.get(11)?,
                hitl_count: row.get(12)?,
                open_pr_count: row.get(13)?,
                prs_needing_attention: row.get(14)?,
                afk_loop_status: row.get(15)?,
                default_branch: row.get(16)?,
                worktree_count: row.get(17)?,
                afk_ready_count: row.get(18)?,
                prd_count: row.get(19)?,
                prd_completed_subs: row.get(20)?,
                prd_total_subs: row.get(21)?,
                status,
            })
        })
