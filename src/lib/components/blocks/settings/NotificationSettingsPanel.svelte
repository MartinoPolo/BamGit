<script lang="ts">
	...
	interface Props {
		...
		showSoundPacks?: boolean;
	}

	let { showEventsTable = true, showSoundPacks = true }: Props = $props();
	...
	{#if showEventsTable}
		...
		<div class="flex items-center gap-3 rounded-lg border border-border bg-muted/30 px-4 py-3">
			...
			<SettingOverrideIndicator
				overridden={settingsCtx.isOverridden('notificationVolume')}
				onreset={() => void settingsCtx.resetOverride('notificationVolume')}
			/>
		</div>
