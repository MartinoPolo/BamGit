...
const MOCK_COMMAND_HANDLERS: Record<string, MockHandler> = {
	...
	open_terminal: () => null,
	open_folder_in_explorer: () => null,
	open_in_editor: () => null,
	update_peacock_color: () => null,
	run_workspace_command: (args) => ({
		process_id: `mock-proc-${Date.now()}`,
		command_id: (args as Record<string, unknown>).command_id as string,
		issue_id: (args as Record<string, unknown>).issue_id as string,
		category: 'check',
		name: 'mock-command',
		pid: 99999,
		port: null,
	...
	test_workspace_command: () => `test-mock-${Date.now()}`,
